    ...
    if backend == StorageBackend.MINIO.value:
        ...
        if not public_endpoint:
            ...
            if endpoint.startswith("minio:"):
                ...
                logger.info(
                    f"✅ Auto-detected Docker MinIO: using {public_endpoint} for browser uploads (internal: {endpoint})"
                )
                ...
                logger.info(
                    f"✅ Auto-detected Docker Desktop: using {public_endpoint} for browser uploads (internal: {endpoint})"
                )
            else:
                # Already using a public endpoint (localhost:9000 or domain:9000)
                public_endpoint = endpoint
                logger.info(
                    f"✅ Using configured endpoint: {public_endpoint} for both API and browser"
                )
