...
class MinioFileSystem(BaseFileSystem):
    ...
    async def aget_presigned_put_url(
        ...
        IMPORTANT: Always use public_endpoint for browser uploads to ensure
        the URL is accessible from outside Docker containers.
        """
        try:
            # Return unsigned URL for anonymous upload
            # ALWAYS use public_endpoint to ensure browser accessibility
            protocol = "https" if self.secure else "http"
            url = f"{protocol}://{self.public_endpoint}/{self.bucket_name}/{file_path}"
            logger.info(f"Generated upload URL for {file_path} -> {url}")
            return url
        except Exception as e:
