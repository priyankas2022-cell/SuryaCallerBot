...
class OpenAIEmbeddingService(BaseEmbeddingService):
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        # OpenAI has a limit on the number of inputs in a single request (usually 2048)
        # We'll use a safer batch size of 100 for better reliability and progress tracking
        batch_size = 100
        all_embeddings = []
        ...
        try:
            for i in range(0, len(texts), batch_size):
                ...
                logger.info(
                    f"Generating OpenAI embeddings for batch {i//batch_size + 1} "
                ...
                # Extract embeddings from response and maintain order
            ...
            logger.exception(f"Error generating OpenAI embeddings: {e}")
            raise
    ...
    async def process_document(
        ...
        try:
            ...
            logger.exception(f"Error processing document with OpenAI: {e}")
