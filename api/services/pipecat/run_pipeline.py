...
async def _run_pipeline(
    ...
    if start_node and start_node.document_uuids:
        if not embeddings_api_key:
            logger.warning(
                f"⚠️  Node '{start_node.name}' has knowledge base documents but no embeddings API key configured. "
                f"Knowledge base will not be accessible. Please configure embeddings in Model Configurations > Embedding."
            )
        else:
            logger.info(
