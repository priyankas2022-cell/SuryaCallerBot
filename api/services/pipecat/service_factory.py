...
# Speechmatics removed - causes import errors when not installed
...
def create_stt_service(
    ...
    if user_config.stt.provider == ServiceProviders.DEEPGRAM.value:
        ...
        # Lazy import Speechmatics to avoid import error when not installed
        try:
            from pipecat.services.speechmatics.stt import SpeechmaticsSTTService
            from pipecat.services.speechmatics.stt import AdditionalVocabEntry
        except ImportError:
            raise HTTPException(
                status_code=503,
                detail="Speechmatics service is not available. Please install required dependencies."
            )
