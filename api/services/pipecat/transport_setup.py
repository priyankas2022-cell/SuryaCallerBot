...
async def create_twilio_transport(
    ...
    config_dict = await load_telephony_config(organization_id)

    if config_dict.get("provider") != "twilio":
        raise ValueError(f"Expected Twilio provider, got {config_dict.get('provider')}")

    account_sid = config_dict.get("account_sid")
