...
async def load_telephony_config(organization_id: int) -> Dict[str, Any]:
    ...
    if account_sid and auth_token and from_number:
        logger.info("Using Twilio configuration from environment variables")
        return {
            "provider": "twilio",
            "account_sid": account_sid,
            "auth_token": auth_token,
            "from_numbers": [from_number],
        }

    raise ValueError(
        f"No telephony configuration found for organization {organization_id} and no environment fallback available"
    )
