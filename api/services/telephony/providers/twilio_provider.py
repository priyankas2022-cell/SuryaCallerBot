...
class TwilioProvider(TelephonyProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        """
        self.account_sid = config.get("account_sid", "").strip() if config.get("account_sid") else None
        self.auth_token = config.get("auth_token", "").strip() if config.get("auth_token") else None
    ...
    async def initiate_call(
        ...
        **kwargs: Any,
        ...
        sid_debug = f"{self.account_sid[:4]}...{self.account_sid[-4:]}" if self.account_sid else "None"
        token_debug = f"{self.auth_token[:4]}...{self.auth_token[-4:]}" if self.auth_token else "None"
        logger.info(f"Initiating Twilio call with SID: {sid_debug} and Token: {token_debug}")
