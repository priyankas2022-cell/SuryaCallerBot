...
def get_chunker(tokenizer):
    global _CHUNKER_CACHE
    if _CHUNKER_CACHE is None:
        logger.info("Initializing HybridChunker (one-time setup)...")
        _CHUNKER_CACHE = HybridChunker(tokenizer=tokenizer)
    return _CHUNKER_CACHE

async def get_tokenizer(max_tokens: int):
    global _TOKENIZER_CACHE
    if TOKENIZER_MODEL not in _TOKENIZER_CACHE:
        logger.info(f"Loading tokenizer: {TOKENIZER_MODEL} (one-time setup)...")
        raw_tokenizer = await asyncio.to_thread(AutoTokenizer.from_pretrained, TOKENIZER_MODEL)
        tokenizer = HuggingFaceTokenizer(
            tokenizer=raw_tokenizer,
            max_tokens=max_tokens,
        )
        _TOKENIZER_CACHE[TOKENIZER_MODEL] = (raw_tokenizer, tokenizer)
    return _TOKENIZER_CACHE[TOKENIZER_MODEL]
...
async def process_knowledge_base_document(
    ...
    try:
        ...
        file_hash = await asyncio.to_thread(db_client.compute_file_hash, temp_file_path)
        mime_type = await asyncio.to_thread(db_client.get_mime_type, temp_file_path)
        filename = s3_key.split("/")[-1]

        # Get document record with a small retry delay to handle potential commit lag
        document = None
        for attempt in range(3):
            document = await db_client.get_document_by_id(document_id)
            if document:
                break
            logger.warning(f"Document {document_id} not found in DB, retrying (attempt {attempt+1}/3)...")
            await asyncio.sleep(2)
            
        if not document:
            raise Exception(f"Document {document_id} not found in database after retries")
        ...
        if document.created_by:
            ...
            if user_config.embeddings:
                ...
                if embeddings_model and embeddings_model.lower() in ["local", "sentence-transformers", "all-minilm-l6-v2"]:
                    ...
                    logger.info(f"Using configured embeddings model: {embeddings_model} (Provider: {getattr(user_config.embeddings, 'provider', 'unknown')})")
        ...
        # Ensure embedding service is ready (loads models if local)
        if hasattr(service, "_get_model"):
            await service._get_model()
        ...
        try:
            raw_tokenizer, tokenizer = await get_tokenizer(max_tokens)
        ...
        if use_docling:
            ...
            try:
                logger.info("Step 1: Converting document with docling (running in thread...)")
                converter = await asyncio.to_thread(get_converter)
                # Add a reasonable timeout for the conversion itself
                conversion_result = await asyncio.wait_for(
                    asyncio.to_thread(converter.convert, temp_file_path),
                    timeout=300 # 5 minutes max for conversion
                )
                doc = conversion_result.document
                logger.info("✅ Document conversion successful")
            except asyncio.TimeoutError:
                logger.error("❌ Document conversion timed out after 5 minutes")
            ...
            logger.info("Step 3: Initializing HybridChunker")
            chunker = await asyncio.to_thread(get_chunker, tokenizer=tokenizer)

            # Step 4: Chunk the document
            logger.info(f"Step 4: Chunking document with max_tokens={max_tokens} (running in thread...)")
            def _chunk():
            ...
            logger.info("Step 5: Processing and contextualizing chunks...")
            for i, chunk in enumerate(dl_chunks):
                chunk_text = chunk.text
                # Contextualizing can be slow for large docs
                contextualized_text = await asyncio.to_thread(chunker.contextualize, chunk=chunk)
            ...
            plain_text = await asyncio.to_thread(_read_as_plain_text, temp_file_path)
            ...
            def _chunk_plain():
                return _chunk_plain_text(plain_text, max_tokens, raw_tokenizer)
            plain_chunks = await asyncio.to_thread(_chunk_plain)
        ...
        total_chunks = len(chunk_records)
        if total_chunks > 0:
            logger.info(f"Generating embeddings for {total_chunks} chunks using {service.get_model_id()}")
            embeddings = await service.embed_texts(chunk_texts)

            # Step 7: Attach embeddings to chunk records
            logger.info("Attaching embeddings to chunk records...")
            for chunk_record, embedding in zip(chunk_records, embeddings):
                chunk_record.embedding = embedding

            # Step 8: Save chunks in database
            logger.info(f"Storing {len(chunk_records)} chunks in database...")
            await db_client.create_chunks_batch(chunk_records)
        else:
            logger.warning(f"No chunks generated for document {document_id}, skipping embedding and storage.")
